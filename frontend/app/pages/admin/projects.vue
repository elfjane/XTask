<template>
  <div class="admin-page-container">
    <div class="page-header">
      ...
      <button v-if="can('manage-projects')" @click="openModal()" class="btn btn-primary">{{ $t('admin.add') }}</button>
    </div>
      ...
      <table class="data-table">
        <thead>
          <tr>
            <th>ID</th>
            <th>Name</th>
            <th v-if="can('manage-projects')">Actions</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="item in items" :key="item.id">
            <td>{{ item.id }}</td>
            <td>{{ item.name }}</td>
            <td v-if="can('manage-projects')" class="actions-cell">
...
async function fetchData() {
  try {
    ...
    const data = await $fetch(`${apiBase}/api/admin/projects`, {
...
async function submitForm() {
  try {
    ...
    const url = editingItem.value
      ? `${apiBase}/api/admin/projects/${editingItem.value.id}`
