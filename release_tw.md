...
## [2026-01-22] - 任務列表 UI 優化
- **前端優化**:
  - **排序按鈕美化**: 將任務列表中的上下箭頭下拉選單替換為現代化的圖標切換按鈕。
    - 使用 SVG 向上/向下箭頭圖標取代純文字符號
    - 新增流暢的懸停動畫效果（放大、陰影、顏色變化）
    - 點擊時提供視覺反饋
    - 顯示 tooltip 提示當前排序方式（升序/降序）
    - 一鍵切換升序/降序排列，提升操作體驗
  - **多國語言修復**: 修正任務狀態「已完成」(completed) 在下拉選單中未正確翻譯的問題，補齊中英文語言文件中缺失的翻譯 key。
  - **版面優化**: 調整電腦版所有主要頁面（任務、排程、個人設定）的標題上方間距，減少不必要的空白，統一全站視覺風格。

## [2026-01-21] - 任務狀態與審核功能增強
- **前端優化**:
  - **送審中標識**: 當任務狀態為「已完成」(`finished`) 並且審核狀態為「送審中」(`submitted`) 時，在桌面版與移動版的任務列表中，「已完成」狀態徽章旁邊會額外顯示一個「送審中」徽章，方便用戶立即識別正在等待審核的完成任務。
  - **拖拉排序功能**: 支援在任務列表中直接拖拉排序 Markdown 連結（如 Google Docs, Asana 連結等）。
  - **跨欄位拖拉**: 用戶現在可以將連結在「備註說明」(Memo) 與「產出連結」(Output/URL) 欄位之間自由拖拉移動。系統會自動更新兩個欄位的內容，極大地方便了任務資料的整理。
  - **詳情頁面佈局調整**: 優化任務詳情彈窗的排版。將核心內容「工作」、「產出的文件或連結」與「備註說明」區塊整合並置頂顯示（位於日期與審核資訊之前），讓重點資訊一目瞭然。
  - **編輯表單排版修復**: 修正任務編輯彈窗中「工作內容」、「產出連結」與「備註」欄位沒有正確佔滿寬度，導致並排顯示的排版錯誤。
  - **編輯表單間距統一**: 將任務編輯彈窗中的日期欄位（發佈日、起始日等）整合至主表單網格中。確保日期欄位的排版間距與部門、狀態等其他欄位完全一致，消除視覺上的不協調感。
- **後端優化**:
  - **自動設定起始日**: 當任務狀態變更為「執行中」(`working`) 時，如果起始日 (`start_date`) 欄位為空白，系統會自動將其設定為當天日期，確保能更精確地追蹤任務實際開始執行的時間。
  - 更新 OpenAPI 規格文檔，說明此自動行為。
  - 新增完整的單元測試，驗證自動設定邏輯並確保已存在的日期不會被覆蓋。

## [2026-01-20] - 系統架構優化與自動化部署
- **自動化部署**:
  - 新增 `deploy.sh` 與 Windows `deploy.bat` 一鍵部署腳本。支援代碼打包、前端構建、SSH 上傳、遠端解壓、資料庫遷移與環境快取清理。
- **效能與 UX 優化**:
  - **加載體驗**: 新增 `SkeletonLoader` 元件。在任務與排程加載時顯示動態骨架屏，顯著提升感官載入速度。
  - **搜尋優化**: 針對列表搜尋實作 Debounce (防抖) 處理（300ms），減少輸入時不必要的 API 請求，減輕後端負擔。
  - **狀態標籤統一化**: 新增 `StatusBadge` 元件，整合所有任務、審核與排程狀態的視覺樣式，並提供完善的深色模式支援。
- **後端重構**:
  - **代碼解法**: 將 Excel 匯入邏輯抽離至 `TaskImportService`，透過更清晰的職責分離（SRP）提升可維護性。
  - **驗證增強**: 全面導入 `FormRequest` 進行資料驗證，讓控制器檔案更精簡且 API 更安全。
- **修復視覺閃爍問題**:
  - 修復了在頁面重整 (F5) 時，大頭照會先以巨大尺寸閃現的問題。透過在大頭照標籤中加入寬高屬性與全域 CSS 樣式修正。
- **依賴更新**:
  - 將 `vue-i18n` 升級至 v11.x 版本，修復了 v9/v10 版本被標記為棄用（Deprecated）的問題。
...
- **後端 (Backend)**:
  ...
  - 更新 API 路由以支援個別任務與排程的獲獲取及完整資料更新。
